...
const FALLBACK = {
  ...
  tools: [
    { id: "chrome", name: "Chrome", description: "快速、安全的网页浏览器", icon: "chrome", entryType: "path", localPath: "/Applications/Google Chrome.app", categoryId: "cat-common", tags: ["浏览器"], isPinned: true, status: "active", sortOrder: 0 },
    { id: "vscode", name: "VS Code", description: "强大的代码编辑器", icon: "vscode", entryType: "path", localPath: "/Applications/Visual Studio Code.app", categoryId: "cat-dev", tags: ["编辑器"], isPinned: true, status: "active", sortOrder: 1 },
    { id: "postman", name: "Postman", description: "API 开发与测试工具", icon: "postman", entryType: "path", localPath: "/Applications/Postman.app", categoryId: "cat-dev", tags: ["API"], isPinned: true, status: "active", sortOrder: 2 },
    ...
    { id: "docker", name: "Docker", description: "构建、运行和管理容器", icon: "docker", entryType: "path", localPath: "/Applications/Docker.app", categoryId: "cat-dev", tags: ["容器"], isPinned: false, status: "active", sortOrder: 4 },
    ...
    { id: "wechat", name: "微信", description: "高效的沟通与协作工具", icon: "wechat", entryType: "path", localPath: "/Applications/WeChat.app", categoryId: "cat-common", tags: ["沟通"], isPinned: false, status: "active", sortOrder: 8 },
    ...
    { id: "typora", name: "Typora", description: "优雅的 Markdown 编辑器", icon: "document", entryType: "path", localPath: "/Applications/Typora.app", categoryId: "cat-local", tags: ["Markdown"], isPinned: false, status: "active", sortOrder: 12 },
    { id: "obsidian", name: "Obsidian", description: "构建你的知识库", icon: "obsidian", entryType: "path", localPath: "/Applications/Obsidian.app", categoryId: "cat-office", tags: ["笔记"], isPinned: false, status: "active", sortOrder: 13 },
  ...
  documents: [
    { id: "doc-product", title: "Tool Hub 产品说明", url: "https://docs.example.com/tool-hub", description: "工具库的产品目标、信息架构和后续规划", categoryId: "cat-office", tags: ["产品", "规划"], isPinned: true, status: "active", sortOrder: 0 },
...
function App() {
  ...
  const openTool = async (tool) => {
    if (tool.entryType === "http") {
      const url = tool.primaryUrl || tool.endpoints?.[0]?.url;
      if (url) window.open(url, "_blank", "noopener,noreferrer"); else showToast("请先配置 HTTP 地址", "error");
    } else await copyValue(tool.localPath, "路径已复制");
  };
  ...
  const createCategory = async () => {
    const name = window.prompt("新分组名称"); if (!name?.trim()) return;
    try { if (usingFallback) setData((current) => ({ ...current, categories: [...current.categories, { id: `local-cat-${Date.now()}`, name: name.trim(), sortOrder: current.categories.length }] })); else { await apiRequest("/api/categories", { method: "POST", body: JSON.stringify({ name: name.trim() }) }); await loadData(); } showToast("分组已创建", "success"); }
    catch (error) { showToast(error.message, "error"); }
  };
  const exportLibrary = async () => {
    try {
      ...
      const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `tool-hub-${new Date().toISOString().slice(0, 10)}.json`; link.click(); URL.revokeObjectURL(link.href);
  ...
  return <div className="app-shell">
    <aside className="icon-rail" aria-label="工作区导航">
      <button className="brand-mark" type="button" onClick={() => { setWorkspace("tools"); setActiveFilter("all"); }} aria-label="Tool Hub"><FiGrid size={28} /></button>
...
function ToolCard({ tool, featured = false, onOpen, onEdit, onDelete, onTogglePinned, menuId, setMenuId, categoryMap }) {
  const category = categoryMap[tool.categoryId]?.name; const address = tool.entryType === "http" ? tool.primaryUrl : tool.localPath;
  return <article className={`tool-card ${featured ? "featured" : ""} ${tool.status !== "active" ? "disabled" : ""}`}><div className="card-topline"><ToolIcon tool={tool} size={featured ? 54 : 46} /><div className="card-actions"><button type="button" className={`star-button ${tool.isPinned ? "selected" : ""}`} aria-label={tool.isPinned ? "取消常用" : "加入常用"} onClick={() => onTogglePinned(tool)}><FiStar size={18} /></button><div className="menu-wrap"><button type="button" className="more-button" aria-label="更多操作" onClick={() => setMenuId(menuId === tool.id ? null : tool.id)}><FiMoreVertical size={18} /></button>{menuId === tool.id && <div className="card-menu"><button type="button" onClick={() => onOpen(tool)}>{tool.entryType === "http" ? <FiArrowUpRight /> : <FiCopy />} {tool.entryType === "http" ? "打开" : "复制路径"}</button><button type="button" onClick={() => onEdit(tool)}><FiEdit3 /> 编辑信息</button><button type="button" onClick={() => onDelete(tool)} className="danger"><FiTrash2 /> 删除</button></div>}</div></div></div><div className="card-content"><h3>{tool.name}</h3><p>{tool.description}</p><div className="card-meta"><span className={`entry-badge ${tool.entryType}`}>{tool.entryType === "http" ? "HTTP" : "本地路径"}</span>{category && <span className="category-badge">{category}</span>}{(tool.tags || []).slice(0, 1).map((tag) => <span className="tag-badge" key={tag}>{tag}</span>)}</div></div><div className="card-bottom"><span className="address-preview" title={address}>{address}</span><button className="open-button" type="button" onClick={() => onOpen(tool)}>{tool.entryType === "http" ? "打开" : "复制路径"}{tool.entryType === "http" ? <FiArrowUpRight size={15} /> : <FiCopy size={15} />}</button></div></article>;
}

function ToolEditor({ editor, categories, onChange, onClose, onSubmit }) {
  const update = (key, value) => onChange((current) => ({ ...current, [key]: value }));
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="editor-modal" role="dialog" aria-modal="true" aria-labelledby="editor-title"><div className="modal-header"><div><p className="eyebrow">TOOL CONFIGURATION</p><h2 id="editor-title">{editor.id ? "编辑工具" : "新增工具"}</h2></div><button type="button" className="icon-button" onClick={onClose} aria-label="关闭"><FiX size={20} /></button></div><form onSubmit={onSubmit}><div className="form-grid"><label>工具名称<input autoFocus value={editor.name} onChange={(event) => update("name", event.target.value)} placeholder="例如：Postman" /></label><label>图标<select value={editor.icon} onChange={(event) => update("icon", event.target.value)}>{Object.keys(toolIcons).map((key) => <option key={key} value={key}>{key}</option>)}</select></label><label className="wide">简介<input value={editor.description} onChange={(event) => update("description", event.target.value)} placeholder="一句话说明这个工具做什么" /></label><label>类型<select value={editor.entryType} onChange={(event) => update("entryType", event.target.value)}><option value="http">HTTP 地址</option><option value="path">本地路径</option></select></label><label>所属分组<select value={editor.categoryId || ""} onChange={(event) => update("categoryId", event.target.value)}><option value="">未分类</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>{editor.entryType === "http" ? <><label className="wide">主地址<input value={editor.primaryUrl} onChange={(event) => update("primaryUrl", event.target.value)} placeholder="192.168.1.10:8080 或 https://example.com" /></label><label className="wide">备用地址 <span className="field-hint">每行一个</span><textarea value={editor.backupUrls} onChange={(event) => update("backupUrls", event.target.value)} rows="2" placeholder="https://backup.example.com" /></label></> : <label className="wide">本地路径<input value={editor.localPath} onChange={(event) => update("localPath", event.target.value)} placeholder="/Users/you/Downloads" /></label>}<label className="wide">标签 <span className="field-hint">用逗号分隔</span><input value={editor.tags} onChange={(event) => update("tags", event.target.value)} placeholder="开发, API" /></label></div><div className="form-options"><label className="check-row"><input type="checkbox" checked={editor.isPinned} onChange={(event) => update("isPinned", event.target.checked)} />加入常用工具</label><label className="check-row"><input type="checkbox" checked={editor.status === "active"} onChange={(event) => update("status", event.target.checked ? "active" : "disabled")} />启用</label></div><div className="modal-footer"><button type="button" className="secondary-button" onClick={onClose}>取消</button><button type="submit" className="primary-button"><FiCheck size={17} />保存工具</button></div></form></section></div>;
}
